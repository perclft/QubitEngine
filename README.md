...
- **Metal Backend Selection**: `MetalBackend` (Apple Silicon GPU) is not part of the automatic `BackendFactory::create` selection chain (which chooses Cloud $\rightarrow$ MPS $\rightarrow$ CUDA $\rightarrow$ CPU); it requires explicit instantiation / dependency injection via `QuantumRegister(n, std::move(metal_backend))`.
